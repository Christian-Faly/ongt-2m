...
import OffertBenefRBMForm from './offertBenefRBMForm';
import * as SQLite from 'expo-sqlite';

export default function OffertBenefRBM({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_benef_rbm",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_benef_rbm"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_benef_rbm WHERE id_benef='+ route.params.id, null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_benef_rbm(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_benef_rbm SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_benef_rbm WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertBenefRBMForm passOffert={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
