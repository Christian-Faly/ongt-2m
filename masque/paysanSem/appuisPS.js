...
import OffertAUEForm from './appuisPSForm';
import * as SQLite from 'expo-sqlite';

// daty, nature, unite, quantite, pu, source_financement
export default function AppuisPS({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_ps",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_ps"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_ps WHERE id_pepiniere='+ route.params.id, null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_ps(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_ps SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_ps WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_ps WHERE id = ?', [id],
