...
import PaysanSemForm from './paysanSemForm';
...
export default function PaysanSem({ route, navigation }) {
  const [comTab, setComTab] = useState([]);
  const [saisie, setSaisie] = useState(false);
  const [exporter, setExporter] = useState(false);
  const activity = 'PaysanSem'
  const [valeurID, setValeurID] = useState(0);
  const [paysanSem, setPaysanSem] = useState([]);
  const [mode, setMode] = useState('Ajouter')
  let currentPaysanSem = undefined
  ...
  useEffect(()=>{
     ...
     db.transaction((tx) => {
  //  tx.executeSql("DROP TABLE  paysan_sem",
      tx.executeSql("CREATE TABLE IF NOT EXISTS paysan_sem"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT, ncommune INTEGER, fokontany TEXT,"+
        "site TEXT, coo_x TEXT, coo_y TEXT, nom_pps TEXT, sexe TEXT, debut_activite DATE)",
          null,
      ()=>console.log('OK'),
      (error)=>console.log('erreur')
    )
  ...
  db.transaction(tx => {
    tx.executeSql("SELECT * FROM paysan_sem ",
      null,
      (txObj, resultSet) => setPaysanSem(resultSet.rows._array),
...
const showMasque = (id) =>{
  if (id===0){
    ...
    setMode('Modifier')
    const nv = [...paysanSem].filter(pepi => pepi.id == id)[0]
    setInitialValue (nv);
  }
  setSaisie(true)
}  

const addPaysanSem = (nouveau) => {
    let insertInto = "INSERT INTO paysan_sem(";
    let values = " VALUES(";
    let tabvalues = [];
    ...
    db.transaction(tx => {
      tx.executeSql(insertInto + values, [currentPaysanSem],
        (txObj, resultSet) => {
          let existingPaysanSem = [...paysanSem];
          nouveau['id'] = resultSet.insertId
          existingPaysanSem.push(nouveau);
          setPaysanSem(existingPaysanSem);
          currentPaysanSem=undefined;
        },
        (txObj, error) => console.log(error)
      );
    });
  ...
  const updatePaysanSem = (donnee) => {
    let update = "UPDATE paysan_sem SET ";  
    ...
    tx.executeSql(update,()=> {},
      (txObj, resultSet) => {
        ...
        if (resultSet.rowsAffected > 0) {
          let existingPaysanSem = [...paysanSem];
          const indexToUpdate = existingPaysanSem.findIndex(pepi => pepi.id === donnee['id']);
          console.log(donnee['id']  +' index  '+indexToUpdate)
          existingPaysanSem[indexToUpdate] = donnee;
          setPaysanSem(existingPaysanSem);
          setSaisie(false)
        }
        else
  ...
  const deleteNom = (id) => {
    db.transaction(tx => {
      tx.executeSql('DELETE FROM paysan_sem WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            let existingPaysanSem = [...paysanSem].filter(pepi => pepi.id !== id);
            setPaysanSem(existingPaysanSem);
          }
        },
        (txObj, error) => console.log(error)
  ...
  console.log(paysanSem)
  return (
    <View style={globalStyles.container}>
      <Card>
        <Text style={globalStyles.titleText}>
          { 'Liste des paysan semencier '}
          {/* navigation.getParam('description') } */}
        </Text>
      </Card>

      <Modal visible={saisie} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            <MaterialIcons 
              ...
              onPress={() => setSaisie(false)} 
            />
            <PaysanSemForm pass={{dataPass,add:addPaysanSem,update:updatePaysanSem}}/>
          </View>
      ...
      <FlatList data={paysanSem} renderItem={({ item }) => (
         <TouchableOpacity> 
          ...
          <View style={styles.container}>
            ...
            <Text style={styles.box}>{ item.nom_pps}</Text>
            <Pressable style={styles.box}
                        onPress={() => {item['titre']= 'Pépinière '
                        navigation.navigate('Exploitation PS', item)}} >
              <Text style={styles.text}>Exploitation</Text>
            </Pressable>
            <Pressable style={styles.box}
                        onPress={() => {item['titre']= 'Pépinière '
                        navigation.navigate('Vente PS', item)}} >
              <Text style={styles.text}>Vente</Text>
            </Pressable>
            <Pressable style={styles.box}
                        onPress={() => {item['titre']= 'Pépinière '
                        navigation.navigate('Appuis PS', item)}} >
              <Text style={styles.text}>Appuis</Text>
            </Pressable>
            <Pressable onPress={() =>{
                                      setValeurID(item.id)
                                      setExporter(true)
                                      let existingPaysanSem = [...paysanSem].filter(pepi => pepi.id !== item.id);
                                      setpepiniere(existingPaysanSem);
                                  }
                                }
                        style={styles.box}> 
