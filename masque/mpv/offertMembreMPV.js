...
import OffertMembreMPVForm from './offertMembreMPVForm';
import * as SQLite from 'expo-sqlite';
import { Alert } from 'react-native';

export default function OffertMembreMPV({ route, navigation }) {
  ...
  const [initialValue, setInitialValue] = useState({ id_membre : route.params.id,
                                                     categorie : '',
                                                     designation : '',
                                                     quantite : 0})
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        // tx.executeSql("DROP TABLE offert_membre_mpv",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_membre_mpv"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_membre INTEGER,"+
        "categorie TEXT,designation TEXT, quantite REAL)",
            null,
        ()=>console.log('OK'),
        (error)=>console.log('erreur')
      )
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_membre_mpv WHERE id_membre='+ route.params.id, null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_membre_mpv(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_membre_mpv SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_membre_mpv WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_membre_mpv WHERE id = ?', [id],
      ...
      db.transaction(tx => {
        tx.executeSql('UPDATE mpv SET nom = ? WHERE id = ?', [currentNom, id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
  ...
  const dataPass ={
    id_membre:route.params.id_membre,
    typa:route.params.typa,
    lstCategorie :dataCategorie,
    lstDesignation :dataDesignation,
    initValue:initialValue,
    speculation_special:route.params.speculation_special,
    titre:': '+route.params.nom +' - '+route.params.titre,
    SPECU:': '+route.params.nom +' - '+route.params.titre,
    mode : mode
  }
  console.log('9999',route.params.speculation_special)  
  return (
    <View style={globalStyles.container}>

      <Card>
        <Text style={globalStyles.titleText}>
          { 'Nom : '+route.params.nom + ' - '+route.params.titre  }
        </Text>
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertMembreMPVForm passOffert={{dataPass,add:addOffert,update:updateOffert}} />
          </View>
