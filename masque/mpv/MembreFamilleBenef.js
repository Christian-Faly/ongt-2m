...
export default function MembreFamilleBenefMPV({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE membre_famille_mpv",
        tx.executeSql("CREATE TABLE IF NOT EXISTS membre_famille_mpv"+
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM membre_famille_cep WHERE id_membre='+ route.params.id, null,
    ...
    const showMasque = (id) =>{
      if (id===0){
        setMode('Ajouter')
        setInitialValue({ id_membre : route.params.id,
                          lien_parente : '',
                          nom_prenom : '',
                          surnom:'',  
                          h_f:'',
