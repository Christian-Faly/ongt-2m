...
import OffertMPVForm from './offertMPVForm';
import * as SQLite from 'expo-sqlite';
import { Alert } from 'react-native';

export default function OffertMPV({ route, navigation }) {
  ...
  const [initialValue, setInitialValue] = useState({ id_mpv : route.params.id,
                                                     categorie : '',
                                                     designation : '',
                                                     quantite : 0
                                                    })
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        // tx.executeSql("DROP TABLE offert_mpv",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_mpv"+
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM offert_mpv WHERE id_mpv='+ route.params.id, null,
    ...
    db.transaction(tx => {
      ...
      let sql = "SELECT typa as categorie FROM type_offert GROUP BY typa "//+" WHERE "+critere+" GROUP BY typa"
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_mpv(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_mpv SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_mpv WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertMPVForm passOffert={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
