...
export default function SpeculationMPV({ navigation }) {
    const [dataSpeculationMPV, setDataSpeculationMPV] = useState([])
    ...
    useEffect(()=>{
        ...
        db.transaction(tx => {
            tx.executeSql('SELECT * FROM speculation_mpv ORDER BY typa', null,
                (txObj, resultSet) => setDataSpeculationMPV(resultSet.rows._array),
    ...
    return (
        ...
        />
    */}
        <FlatList data={dataSpeculationMPV} renderItem={({ item }) => (
          <TouchableOpacity 
            onPress={() => navigation.navigate('Micro Projet Vulnerable (MPV)', item)}
