...
import OffertMembreCEPForm from './offertMembreCEPForm';
import * as SQLite from 'expo-sqlite';
import { Alert } from 'react-native';

export default function OffertMembre({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        // tx.executeSql("DROP TABLE offert_membre_cep",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_membre_cep"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_membre_cep WHERE id_membre='+ route.params.id, null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_membre_cep(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_membre_cep SET ";  
    ...
    const deleteNom = (id) => {
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_membre_cep WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertMembreCEPForm passOffert={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
