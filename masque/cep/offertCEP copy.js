...
export default function OffertCEP({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        ...
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_membre_cep INTEGER,"+
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM offert_cep WHERE id_membre_cep='+ route.params.id, null,
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        tx.executeSql('SELECT * FROM offert_cep WHERE id = ?', [id],
      ...
      db.transaction(tx => {
        tx.executeSql('UPDATE cep SET nom = ? WHERE id = ?', [currentNom, id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
