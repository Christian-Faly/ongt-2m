...
import BenefAUEForm from './benefAUEForm';
import * as SQLite from 'expo-sqlite';

export default function BenefAUE({ route,navigation }) {
  ...
  useEffect(()=>{
      ...
      db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE benef_aue",
        tx.executeSql("CREATE TABLE IF NOT EXISTS benef_aue"+
          "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_aue INTEGER,ncommune INTEGER,"+
          "fokontany TEXT,village TEXT, nom TEXT, surnom TEXT, h_f TEXT, annee_naissance INTEGER,"+
          "cin TEXT, statut_menage TEXT, type_fonction TEXT,mode_faire_valoir TEXT, surface REAL,"+
          "fonction TEXT)",
        null,
        ()=>console.log('OK'),
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM benef_aue WHERE id_aue='+route.params.id ,null,
    ...
    const addBenef = (nouveau) => {
        let insertInto = "INSERT INTO benef_aue(";
        let values = " VALUES(";
        let tabvalues = [];
        ...
        db.transaction(tx => {
          tx.executeSql(insertInto + values, [currentBenef],
            (txObj, resultSet) => {
              let existingBenefAUE = [...benef];
              nouveau['id'] = resultSet.insertId
              existingBenefAUE.push(nouveau);
              setBenef(existingBenefAUE);
      ...
      const updateBenef = (donnee) => {
        let update = "UPDATE benef_aue SET ";  
        ...
        tx.executeSql(update,()=> {},
          (txObj, resultSet) => {
            ...
            if (resultSet.rowsAffected > 0) {
              let existingBenefAUE = [...benef];
              const indexToUpdate = existingBenefAUE.findIndex(benef => benef.id === donnee['id']);
              console.log(donnee['id']  +' index  '+indexToUpdate)
              existingBenefAUE[indexToUpdate] = donnee;
              setBenef(existingBenefAUE);
    ...
    console.log('id',id)  
    db.transaction(tx => {
      tx.executeSql('DELETE FROM benef_aue WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            console.log('mandalo2')
            let existingBenefAUE = [...benef].filter(benef => benef.id !== id);
            setBenef(existingBenefAUE);
          }
        },
        (txObj, error) => console.log(error)
      ...
      <Modal visible={saisieBenef} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <BenefAUEForm passBenef={{dataPass,add:addBenef,update:updateBenef,addBenef:addBeneficiaire}}/>
          </View>
