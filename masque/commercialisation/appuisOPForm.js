...
export default function AppuisPPForm({ pass }) {
  ...
  Object.entries(pass.dataPass.initValue).forEach(([key, value]) => {
    ...
    const  lstSourceFinancement =[
      {key:'Projet',value :'Projet' },
      {key:'OM',value :'OM' }
    ]
    const [selectedSourceFinancement, setSelectedSourceFinancement] = useState(pass.dataPass.initValue.source_financement);
  ...
  const submitForm = () => {
    form.source_financement = selectedSourceFinancement
    form.typa = selectedTypa
  ...
  return (
    ...
    <Text style={FormStyles.paragraph}>{'Saisie Appuis OP commerciale'} </Text>
    </Card>
    ...
    <Card style={FormStyles.card}>
      ...
      <View style={{elevation:50,zIndex: 5, position: 'relative'}}>
          ...
          <SelectList data = {lstSourceFinancement}  setSelected = {setSelectedSourceFinancement}
                      save='value'
                      defaultOption = {{key:selectedSourceFinancement,value:selectedSourceFinancement}}
                      value={selectedSourceFinancement}
                      placeholder='-------------'
