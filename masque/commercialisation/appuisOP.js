...
import AppuisOPForm from './appuisOPForm';
import * as SQLite from 'expo-sqlite';

export default function AppuisOP({ route, navigation }) {
  ...
  const [initialValue, setInitialValue] = useState({id_op : route.params.id,
    source_financement : '',                                                
    daty: '', 
    typa : '',
    nature : '', 
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_op",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_op"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_op INTEGER,"+
        "source_financement TEXT, daty DATE, typa TEXT, nature TEXT, unite TEXT, quantite REAL, pu REAL)",
            null,
        ()=>console.log('OK'),
        (error)=>console.log('erreur')
      )
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_op WHERE id_op='+ route.params.id, null,
    ...
    const showMasque = (id) =>{
      if (id===0){
        setMode('Ajouter')
        setInitialValue({id_op : route.params.id,
          source_financement : '',                                                
          daty: '', 
          typa : '',
          nature : '', 
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_op(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_op SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_op WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_op WHERE id = ?', [id],
    ...
    <View style={globalStyles.container}>
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <AppuisOPForm pass={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
      ...
      <FlatList data={offert} renderItem={({ item }) => (
          <View style={styles.row}>
            ...
            <Text style={styles.box}>{ item.typa +' '+item.nature +' '+transformDate(item.daty)}</Text>
            <Pressable style={styles.box}
