...
import VenteBIForm from './venteBIForm';

// VenteAUE == VenteBI
export default function VenteBI({ route,navigation }) {
  ...
  useEffect(()=>{
    //Période, Essence vendue, Quantité,PU, Destinataire
      ...
      db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE vente_bi",
        tx.executeSql("CREATE TABLE IF NOT EXISTS vente_bi"+
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM vente_bi WHERE id_pepiniere='+route.params.id ,null,
    ...
    const addVente = (nouveau) => {
        let insertInto = "INSERT INTO vente_bi(";
        let values = " VALUES(";
        let tabvalues = [];
        ...
        db.transaction(tx => {
          tx.executeSql(insertInto + values, [currentVente],
            (txObj, resultSet) => {
              let existingVenteBI = [...vente];
              nouveau['id'] = resultSet.insertId
              existingVenteBI.push(nouveau);
              setVente(existingVenteBI);
      ...
      const updateVente = (donnee) => {
        let update = "UPDATE vente_bi SET ";  
        ...
        tx.executeSql(update,()=> {},
          (txObj, resultSet) => {
            ...
            if (resultSet.rowsAffected > 0) {
              let existingVenteBI = [...vente];
              const indexToUpdate = existingVenteBI.findIndex(vente => vente.id === donnee['id']);
              console.log(donnee['id']  +' index  '+indexToUpdate)
              existingVenteBI[indexToUpdate] = donnee;
              setVente(existingVenteBI);
    ...
    console.log('id',id)  
    db.transaction(tx => {
      tx.executeSql('DELETE FROM vente_bi WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            console.log('mandalo2')
            let existingVenteBI = [...vente].filter(vente => vente.id !== id);
            setVente(existingVenteBI);
          }
        },
        (txObj, error) => console.log(error)
      ...
      <Modal visible={saisieVente} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <VenteBIForm pass={{dataPass,add:addVente,update:updateVente}}/>
          </View>
