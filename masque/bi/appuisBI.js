...
import OffertAUEForm from './appuisBIForm';
import * as SQLite from 'expo-sqlite';

// daty, nature, unite, quantite, pu, source_financement
export default function AppuisBI({ route, navigation }) {
  ...
  const [initialValue, setInitialValue] = useState({id_bi : route.params.id,
    daty: '', 
    nature : '', 
    unite : '', 
    quantite : '', 
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_bi",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_bi"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_bi INTEGER,"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_bi WHERE id_bi='+ route.params.id, null,
    ...
    const showMasque = (id) =>{
      if (id===0){
        setMode('Ajouter')
        setInitialValue({id_bi : route.params.id,
          daty: '', 
          nature : '', 
          unite : '', 
          quantite : '', 
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_bi(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_bi SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_bi WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_bi WHERE id = ?', [id],
