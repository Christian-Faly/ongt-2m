...
export default function BI({ route, navigation }) {
  ...
  //id , ncommune, fokontany, site TEXT, coo_x TEXT, coo_y TEXT, proprietaire TEXT, sexe TEXT, date
  ...
  useEffect(()=>{
     ...
     db.transaction((tx) => {
  //  tx.executeSql("DROP TABLE  bi",
      tx.executeSql("CREATE TABLE IF NOT EXISTS bi"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT, ncommune INTEGER, fokontany TEXT,"+
        "site TEXT, coo_x TEXT, coo_y TEXT, proprietaire TEXT, sexe TEXT, debut_activite DATE)",
          null,
      ()=>console.log('OK'),
      (error)=>console.log('erreur')
    )
  ...
  db.transaction(tx => {
    tx.executeSql("SELECT * FROM bi ",
      null,
      (txObj, resultSet) => setBI(resultSet.rows._array),
...
const showMasque = (id) =>{
  if (id===0){
    ...
    setMode('Modifier')
    const nv = [...bi].filter(pepi => pepi.id == id)[0]
    setInitialValue (nv);
  }
  setSaisie(true)
}  
...
const addBI = (nouveau) => {
    ...
    db.transaction(tx => {
      tx.executeSql(insertInto + values, [currentBI],
        (txObj, resultSet) => {
          let existingBI = [...bi];
          nouveau['id'] = resultSet.insertId
          existingBI.push(nouveau);
          setBI(existingBI);
          currentBI=undefined;
        },
        (txObj, error) => console.log(error)
  ...
  const updateBI = (donnee) => {
    let update = "UPDATE bi SET ";  
    ...
    tx.executeSql(update,()=> {},
      (txObj, resultSet) => {
        ...
        if (resultSet.rowsAffected > 0) {
          let existingBI = [...bi];
          const indexToUpdate = existingBI.findIndex(pepi => pepi.id === donnee['id']);
          console.log(donnee['id']  +' index  '+indexToUpdate)
          existingBI[indexToUpdate] = donnee;
          setBI(existingBI);
          setSaisie(false)
        }
        else
  ...
  const deleteNom = (id) => {
    db.transaction(tx => {
      tx.executeSql('DELETE FROM bi WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            let existingBI = [...bi].filter(pepi => pepi.id !== id);
            setBI(existingBI);
          }
        },
        (txObj, error) => console.log(error)
  ...
  return (
    <View style={globalStyles.container}>
      ...
      <Modal visible={saisie} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <BIForm pass={{dataPass,add:addBI,update:updateBI}}/>
          </View>
      ...
      <FlatList data={bi} renderItem={({ item }) => (
         <TouchableOpacity> 
          ...
          <View style={styles.container}>
            ...
            <Text style={styles.box}>{ item.proprietaire}</Text>
            <Pressable style={styles.box}
            ...
            <Pressable style={styles.box}
                        onPress={() => {item['titre']= 'Boutique intrant '
                        navigation.navigate('Vente BI', item)}} >
              <Text style={styles.text}>Vente</Text>
            </Pressable>
            <Pressable style={styles.box}
                        onPress={() => {item['titre']= 'Boutique intrant '
                        navigation.navigate('Appuis BI', item)}} >
              <Text style={styles.text}>Appuis</Text>
            </Pressable>
            <Pressable onPress={() =>{
                                      setValeurID(item.id)
                                      setExporter(true)
                                      let existingBI = [...bi].filter(pepi => pepi.id !== item.id);
                                      setbi(existingBI);
                                  }
                                }
                        style={styles.box}> 
