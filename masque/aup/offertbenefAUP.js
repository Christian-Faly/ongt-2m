...
import OffertBenefAUPForm from './offertBenefAUPForm';
import * as SQLite from 'expo-sqlite';

export default function OffertBenefAUP({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_benef_aup",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_benef_aup"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_benef_aup WHERE id_benef='+ route.params.id, null,
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT typa as categorie FROM type_offert GROUP BY typa', null,
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT typa as categorie, designation FROM type_offert', null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_benef_aup(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_benef_aup SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_benef_aup WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_benef_aup WHERE id = ?', [id],
      ...
      db.transaction(tx => {
        tx.executeSql('UPDATE aup SET nom = ? WHERE id = ?', [currentNom, id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
    ...
    <View style={globalStyles.container}>
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertBenefAUPForm passOffert={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
      ...
      <FlatList data={offert} renderItem={({ item }) => (
          <View style={styles.row}>
            ...
            <Text style={styles.box}>{ item.designation }</Text>
