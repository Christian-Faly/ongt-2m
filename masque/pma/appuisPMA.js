...
import OffertAUEForm from './appuisPMAForm';
import * as SQLite from 'expo-sqlite';

// daty, nature, unite, quantite, pu, source_financement
export default function AppuisPMA({ route, navigation }) {
  ...
  const [initialValue, setInitialValue] = useState({id_pma : route.params.id,
    daty: '', 
    nature : '', 
    unite : '', 
    quantite : '', 
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE offert_pma",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_pma"+
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_pma INTEGER,"+
        "daty DATE, nature TEXT, unite TEXT, quantite REAL, pu REAL, source_financement TEXT)",
            null,
        ()=>console.log('OK'),
        (error)=>console.log('erreur')
      )
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_pma WHERE id_pma='+ route.params.id, null,
    ...
    const showMasque = (id) =>{
      if (id===0){
        setMode('Ajouter')
        setInitialValue({id_pma : route.params.id,
          daty: '', 
          nature : '', 
          unite : '', 
          quantite : '', 
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_pma(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_pma SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_pma WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
    ...
    const consulter = (id) => {
      db.transaction(tx => {
        console.log('mandalo2')
        tx.executeSql('SELECT * FROM offert_pma WHERE id = ?', [id],
