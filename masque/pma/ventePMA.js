...
import VentePMAForm from './ventePMAForm';

// VenteAUE == VentePMA
export default function VentePMA({ route,navigation }) {
  ...
  useEffect(()=>{
    //Période, Essence vendue, Quantité,PU, Destinataire
      ...
      db.transaction((tx) => {
        //  tx.executeSql("DROP TABLE vente_pma",
        tx.executeSql("CREATE TABLE IF NOT EXISTS vente_pma"+
          "(id INTEGER PRIMARY KEY AUTOINCREMENT,id_pma INTEGER,annee INTEGER,"+
          "periode TEXT, pma_vendue TEXT, quantite REAL,pu REAL, destinataire TEXT)",
        null,
        ()=>console.log('OK'),
        (error)=>console.log('erreur')
      )
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM vente_pma WHERE id_pma='+route.params.id ,null,
    ...
    const addVente = (nouveau) => {
        let insertInto = "INSERT INTO vente_pma(";
        let values = " VALUES(";
        let tabvalues = [];
        ...
        db.transaction(tx => {
          tx.executeSql(insertInto + values, [currentVente],
            (txObj, resultSet) => {
              let existingVentePMA = [...vente];
              nouveau['id'] = resultSet.insertId
              existingVentePMA.push(nouveau);
              setVente(existingVentePMA);
      ...
      const updateVente = (donnee) => {
        let update = "UPDATE vente_pma SET ";  
        ...
        tx.executeSql(update,()=> {},
          (txObj, resultSet) => {
            ...
            if (resultSet.rowsAffected > 0) {
              let existingVentePMA = [...vente];
              const indexToUpdate = existingVentePMA.findIndex(vente => vente.id === donnee['id']);
              console.log(donnee['id']  +' index  '+indexToUpdate)
              existingVentePMA[indexToUpdate] = donnee;
              setVente(existingVentePMA);
    ...
    console.log('id',id)  
    db.transaction(tx => {
      tx.executeSql('DELETE FROM vente_pma WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            console.log('mandalo2')
            let existingVentePMA = [...vente].filter(vente => vente.id !== id);
            setVente(existingVentePMA);
          }
        },
        (txObj, error) => console.log(error)
    ...
    <View style={globalStyles.container}>
      ...
      <Modal visible={saisieVente} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <VentePMAForm pass={{dataPass,add:addVente,update:updateVente}}/>
          </View>
      ...
      <FlatList data={vente} renderItem={({ item }) => (
          <View style={styles.container}>
            ...
            <Text style={styles.box}>{item.periode+' - '+item.pma_vendue}</Text>
            {/* <Pressable style={styles.box}
                        onPress={() => {
