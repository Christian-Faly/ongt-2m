...
import { StyleSheet, View, Text, TouchableOpacity, FlatList, Modal,
  TouchableWithoutFeedback, Keyboard,TextInput, Button ,Pressable,Dimensions} from 'react-native';
// import { StyleSheet, Text, View, TextInput, Button, Platform } from 'react-native';
import { globalStyles  } from '../../styles/global';
import { MaterialIcons,MaterialCommunityIcons,Entypo } from '@expo/vector-icons';
import Card from '../../shared/card';
import OffertBenefFAForm from './offertBenefFAForm';
import * as SQLite from 'expo-sqlite';

export default function OffertBenefFA({ route, navigation }) {
  ...
  useEffect(()=>{
    ...
    db.transaction((tx) => {
        // tx.executeSql("DROP TABLE offert_benef_fa",
        tx.executeSql("CREATE TABLE IF NOT EXISTS offert_benef_fa"+
    ...
    db.transaction(tx => {
      ...
      tx.executeSql('SELECT * FROM offert_benef_fa WHERE id_benef='+ route.params.id, null,
    ...
    const addOffert = (nouveau) => {
        let insertInto = "INSERT INTO offert_benef_fa(";
        let values = " VALUES(";
        let tabvalues = [];
      ...
      const updateOffert = (donnee) => {
        let update = "UPDATE offert_benef_fa SET ";  
    ...
    const deleteNom = (id) => {
      ...
      db.transaction(tx => {
        tx.executeSql('DELETE FROM offert_benef_fa WHERE id = ?', [id],
          (txObj, resultSet) => {
            if (resultSet.rowsAffected > 0) {
              console.log('mandalo2')
      ...
      <Modal visible={saisieOffert} animationType='slide'>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.modalContent}>
            ...
            />
            <OffertBenefFAForm passOffert={{dataPass,add:addOffert, update:updateOffert}} />
          </View>
      ...
      <FlatList data={offert} renderItem={({ item }) => (
          <View style={styles.container}>
            <Pressable style={styles.box}
                       onPress={() => {showMasque(item.id),                                       setValueToUpdate({id:100})}}>
